...
function DailyExpensesForm(props) {
  ...
  const [monthlyBudget, setMonthlyBudget] = useState(
    calculateTotal(propBudgetData.earnings) -
      calculateTotal(propBudgetData.expenses)
  );
  ...
  const [lastDay, setLastDay] = useState(
    new Date(
      new Date().setDate(new Date().getDate() + (((-dayToday - 2) % 7) + 6))
    )
  );

  const [firstDayISO, setFirstDayISO] = useState(
    firstDay.toISOString().slice(0, 10)
  );
  const [lastDayISO, setLastDayISO] = useState(
  ...
  const [dailyExpensesArr, setDailyExpensesArr] = useState(
    propDailyExpensesData.filter(
      (element) =>
        element.date.slice(0, 10) >= firstDayISO &&
        element.date.slice(0, 10) <= lastDayISO
    )
  );

  const [dailyExpensesTotal, setdailyExpensesTotal] = useState(
    calculateTotal(dailyExpensesArr)
  );
  const [budgetTotal, setBudgetTotal] = useState((monthlyBudget / 31) * 7);
  const [budgetLeft, setBudgetLeft] = useState(
    budgetTotal - dailyExpensesTotal
  );

  //PIE chart stuff
  const categoriesArr = propBudgetData.spendingCategories;
  // ['Food', 'Activities', 'Other']
  ...
  useEffect(() => {
    ...
    timePeriod === "week"
      ? setBudgetTotal((monthlyBudget / 31) * 7)
      : setBudgetTotal(monthlyBudget);
  ...
  useEffect(() => {
    ...
    let newFirstDay = new Date(
      currentFirstDay.getTime() + numOfItemsToNavigate * oneWeek
    );
    let newLastDay = new Date(
      currentLastDay.getTime() + numOfItemsToNavigate * oneWeek
    );
    ...
    if (timePeriod === "month") {
      ...
      newFirstDay = new Date(
        yearToday,
        monthToday + numOfItemsToNavigate,
        1,
        timezoneOffsetHours
      );
      newLastDay = new Date(
        yearToday,
        monthToday + numOfItemsToNavigate + 1,
        0,
        timezoneOffsetHours
      );
    }
    ...
    setIsCurrentTime(
      (timePeriod === "month" &&
        yearToday === +lastDayISO.slice(0, 4) &&
        monthToday + 1 === +lastDayISO.slice(5, 7)) ||
        (timePeriod === "week" &&
          new Date().toISOString() >= firstDayISO &&
          new Date().toISOString() <= lastDayISO)
        ? true
        : false
    );
  }, [
  ...
  const handleAddDailyExpense = async (event) => {
    ...
    setDailyExpensesArr(
      [newDailyExpense, ...dailyExpensesArr].sort((a, b) =>
        a.date > b.date ? -1 : b.date > a.date ? 1 : 0
      )
    );
    setdailyExpensesTotal(
      calculateTotal([newDailyExpense, ...dailyExpensesArr])
    );
    setBudgetLeft(
      budgetTotal - calculateTotal([newDailyExpense, ...dailyExpensesArr])
    );
          ...
          <table className="table-daily-expenses">
                    ...
                    <tr key={dailyExpense._id}>
                      ...
                      <td>
                        <button
                          ...
                          onClick={(event) =>
                            handleDeleteDailyExpense(index, event)
                          }>
                          –
                        </button>
                      </td>
            ...
            <tfoot>
              <tr>
                ...
                <td></td>
                <td>
                  -{(budgetTotal - budgetLeft).toFixed(2)}{" "}
                  {propBudgetData.currency}
                </td>
              </tr>
            </tfoot>
